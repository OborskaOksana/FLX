...
function userCard(key) {
    ...
    function takeCredits(balance) {
      const minBalance = 0;
      const newBalance = this.card3.balance - balance;
    ...
    function transferCredits(amount, userCard) {
      const minBalance = 0;
      ...
      if (newBalance < minBalance || newBalance > this.card.transactionLimit) {
        ...
        userCard.putCredits(amountWithComision);
      }
    }
  ...
  function UserAccount() {
    ...
    this.getCardByKey = function (key) {
      ...
      return this.cards.find(function(element) {
  
        return element.card.key === key
      });
    }
